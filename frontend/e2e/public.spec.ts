...
test.describe('공개 페이지', () => {
  test('포트폴리오 → 메뉴 → 글 목록 → 첫 글 상세 → 뒤로', async ({ page }) => {
    ...
    const firstCard = page.locator('main .drop-shadow-md').first();
    const firstTitle = await firstCard.locator('.text-2xl').innerText();
    await firstCard.click();
    await expect(page).toHaveURL(/\/post\/\d+$/);
    await expect(page.locator('.text-4xl')).toHaveText(new RegExp(firstTitle.trim()));
